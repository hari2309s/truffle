...
export const translations: Record<Locale, Translations> = {
  ...
  ja: {
    auth: {
      tagline: 'あなたの家計を、明らかに。',
      emailPlaceholder: 'メールアドレス',
      continueWithEmail: 'メールで続ける',
      sending: '送信中...',
      footer: 'マジックリンクでログイン · パスワード不要',
      checkEmail: 'メールをご確認ください',
      magicLinkSent: (email) =>
        `${email}にマジックリンクを送信しました。クリックしてログイン — パスワード不要。`,
    },
    onboarding: {
      heading: 'Truffleへようこそ',
      subtitle: '30秒でセットアップできます。',
      nameLabel: 'お名前を教えてください',
      namePlaceholder: 'お名前',
      currencyLabel: '通貨',
      saving: '保存中...',
      continue: '続ける →',
      ...
      steps: [
        {
          emoji: '💬',
          title: '自然に話しかけて',
          body: 'Truffleに友達にメッセージするように話しかけてください。状況を確認したり、支出を記録したり、アドバイスを求めたり — 音声でもテキストでも。',
        },
        {
          emoji: '💴',
          title: 'すべての支出を記録',
          body: 'すべての取引が自動で分類されます。チャット、CSVインポート、レシート撮影で追加できます。',
        },
        {
          emoji: '🔔',
          title: 'スマートな通知',
          body: 'Truffleは予算超過、異常な支出、貯蓄ストリークを監視し — 問題が起きる前に教えてくれます。',
        },
      ],
      next: '次へ',
      letsGo: '始める →',
    ...
    settings: {
      title: '設定',
      yourData: 'あなたのデータ',
      downloadData: 'すべてのデータをダウンロード',
      downloadDataDesc: '取引、目標、予算、習慣をJSONでエクスポート',
      exportData: 'データをエクスポート',
      preparing: '準備中…',
      dangerZone: '危険な操作',
      deleteAccount: 'アカウント削除',
      deleteAccountDesc: 'すべてのデータを完全に削除します。元に戻せません。',
      deletePlaceholder: '「削除」と入力して確認',
      deleteConfirmWord: '削除',
      deleting: '削除中…',
      deleteMyAccount: 'アカウントを削除する',
      deleteError: 'アカウントの削除に失敗しました。もう一度お試しください。',
      language: '言語',
      currency: '通貨',
    },
    dashboard: {
    ...
    home: {
      authExpiredError:
        'ログインリンクが期限切れか既に使用されています。新しいリンクをリクエストしてください。',
    },
    categories: {
      food_groceries: '食料品',
    ...
    transactions: {
      ...
      clear: 'クリア',
      exportCSV: 'CSVエクスポート',
      noMatching: '一致する取引がありません。',
      noThisMonth: '今月の取引はまだありません。',
      loadMonth: (label) => `${label}を読み込む`,
      deleteConfirm: 'この取引を削除しますか？',
      cancel: 'キャンセル',
      delete: '削除',
      descriptionPlaceholder: '説明',
      merchantPlaceholder: 'お店（任意）',
      save: '保存',
    },
    csvImport: {
      title: 'CSVインポート',
      hint: '日付、説明、金額',
      tapToSelect: 'タップしてCSVファイルを選択',
      parseError: 'CSVを解析できませんでした。日付、説明、金額の列があることを確認してください。',
      transactionsFound: (n) => `${n}件の取引が見つかりました — インポートするものを選択`,
      selectedOf: (selected, total) => `${total}件中${selected}件を選択`,
      showLess: '少なく表示',
      showMore: (n) => `+${n}件`,
      cancel: 'キャンセル',
      import: (n) => `${n}件をインポート`,
      importing: 'インポート中…',
      imported: (n) => `${n}件の取引をインポートしました`,
      done: '完了',
    ...
    receiptUpload: {
      title: 'レシート・明細書をスキャン',
      hint: '画像またはPDF',
      tapToSelect: 'タップしてレシートまたは明細書を選択',
      fileTypes: 'JPEG · PNG · WEBP · PDF',
      invalidFile: '画像（JPEG、PNG、WEBP）またはPDFを選択してください。',
      fileTooLarge: (maxMb) => `ファイルが大きすぎます — 最大${maxMb}MBです。`,
      changeFile: 'ファイルを変更',
      analysing: '分析中…',
      extractTransactions: '取引を抽出',
      transactionsFound: (n) => `${n}件の取引が見つかりました — インポート前に確認`,
      tryAgain: 'もう一度試す',
      import: (n) => `${n}件をインポート`,
      importing: 'インポート中…',
      imported: (n) => `${n}件の取引をインポートしました`,
      done: '完了',
      noTransactions: '取引が見つかりませんでした。レシートが読みやすいことを確認してください。',
      parseError: 'ファイルを解析できませんでした。より鮮明な画像をお試しください。',
      importFailed: 'インポートに失敗しました。もう一度お試しください。',
    ...
    weeklySummary: {
      ...
      readAloud: '読み上げ',
      playing: '再生中…',
      spent: (amount) => `今週${amount}を使いました`,
      earned: (amount) => `、${amount}を稼ぎました`,
      topCategory: (label) => `。${label}が最も多かったです。`,
    },
    financialBrief: {
      addTransactions: '財務概要を見るには取引を追加してください',
      endOfMonth: '月末',
      projectedBalance: '予測残高',
      current: '現在',
      addForForecast: '予測を見るには今月の取引を追加してください',
      income: '収入',
      expenses: '支出',
      netBalance: '純残高',
      noDataThisMonth: '今月のデータなし',
      confidence: { high: '精度：高', medium: '精度：中', low: '精度：低' },
    },
    filter: {
    ...
    insights: {
      title: 'インサイト',
      monthForecast: '月次予測',
      addTransactionsForForecast: '予測を見るには取引を追加してください',
      spendingCalendar: '支出カレンダー',
      monthlyBudgets: '月次予算',
      newBudget: '+ 新しい予算',
      savingsGoals: '貯蓄目標',
      newGoal: '+ 新しい目標',
      savingHabits: '貯蓄習慣',
      recurringSubscriptions: '定期サブスクリプション',
      lastCharged: '最終請求',
      detected: (months) => `${months}ヶ月検出`,
      thingsToReview: '確認すべき事項',
      noUnusualActivity: '異常な活動は検出されていません',
      projectedEndOfMonth: '月末予測',
      perMonth: '/月',
    },
    savingsGoals: {
      title: '貯蓄目標',
      newGoal: '+ 新しい目標',
      cancel: 'キャンセル',
      noGoals: 'まだ目標がありません — 目標を設定して貯蓄を始めましょう',
      deadlinePassed: '期限切れ',
      daysLeft: (days) => `残り${days}日`,
      complete: '達成！',
      addFunds: '+ 入金',
      add: '追加',
      goalNamePlaceholder: '目標名（例：東京旅行）',
      targetAmount: '目標金額',
      creating: '作成中…',
      createGoal: '目標を作成',
    },
    savingsHabits: {
      noHabits: 'まだ貯蓄習慣がありません — Truffleに設定を依頼してください',
      periodWeek: '週',
      periodMonth: '月',
      totalSaved: (amount) => `${amount}合計貯蓄`,
      done: '✓ 完了',
      log: '+ 記録',
      frequencyWeekly: '週次',
      frequencyMonthly: '月次',
    ...
    monthlyBudgets: {
      noBudgets: 'まだ予算がありません — カテゴリ別支出を追跡するために設定してください',
      allCategoriesHaveBudget: 'すべての支出カテゴリに予算が設定されています。',
      over: (amount) => `${amount}超過`,
      left: (amount) => `${amount}残り`,
      budgetLabel: (amount) => `予算 ${amount}/月`,
      monthlyLimit: '月次上限',
      saving: '保存中…',
      setBudget: '予算を設定',
      removeBudget: '予算を削除',
    ...
    topBar: {
      subtitle: 'なんでも聞いてください',
      signOut: 'サインアウト',
      settingsLabel: '設定',
    },
    chat: {
      holdButton: 'ボタンを押したまま、なんでも聞いてください。',
      suggestions: ['今月の状況は？', '食費はいくら使った？', '週末旅行は行ける？'],
      typePlaceholder: 'または質問を入力...',
      stop: '停止',
      mute: '音声をミュート',
      unmute: '音声をミュート解除',
    ...
    proposals: {
      goal: {
        addToGoals: 'この目標を追加しますか？',
        noThanks: 'いいえ、結構です',
        yesAddIt: 'はい、追加する',
        saving: '保存中…',
        error: '問題が発生しました — もう一度お試しください。',
        addedToGoals: (emoji, name) =>
          `${emoji} ${name}を目標に追加しました — インサイトで確認できます。`,
      },
      transaction: {
        logThis: 'この取引を記録しますか？',
        noThanks: 'いいえ、結構です',
      ...
      habit: {
        weeklyLabel: '週次貯蓄習慣',
        monthlyLabel: '月次貯蓄習慣',
        logEachPeriod: (period) => `インサイトで${period}ごとに自分で記録します`,
        notNow: '今は不要',
        startSaving: '貯蓄を始める',
        settingUp: '設定中…',
        error: '問題が発生しました — もう一度お試しください。',
